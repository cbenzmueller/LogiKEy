theory max imports Main       (* Quasi-maximality: C. Benzmüller & X. Parent, 2020 *)
...
lemma assumes "\<forall>x y. (tc R x y \<longrightarrow> R y x)" 
...
abbreviation Suzumara
  where "Suzumara R \<equiv> \<forall>x y. (tc R x y \<longrightarrow> \<not> R y x)"
