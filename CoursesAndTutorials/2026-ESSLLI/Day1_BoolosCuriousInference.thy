...
lemma "\<^bold>d\<^bold>f\<^bold>s\<^bold>s\<^bold>s\<^bold>s\<^bold>e\<^bold>s\<^bold>s\<^bold>s\<^bold>s\<^bold>e" \<comment>\<open>Trying to prove automatically\<close> 
  oops  \<comment>\<open>no proof found; timeout\<close>


text\<open>We interactively provide two shorthand notations ind and p. After their
introduction a proof can be found fully automatically with \textit{Sledgehammer}. ind X
is defined to hold if  and only if  X is `inductive' over  e and  s. pxy holds if and only if pxy 
is in smallest inductive set over  e and  s. Note that the symbols ind and p do not occur in 
...
text\<open>Automated Proof: Using a Single Definition\<close>
...
section\<open>Proof Reconstruction: we reconstruct and verify in Isabelle/HOL the proof argument found by E 
...
text \<open>Lemma L2 can actually be simplified (this was also hinted at by an anonymous 
...
text \<open>Instead of using L2 we can now use L2_5 in our proof of BCP from above:\<close>
theorem "\<^bold>d\<^bold>f\<^bold>s\<^bold>s\<^bold>s\<^bold>s\<^bold>e\<^bold>s\<^bold>s\<^bold>s\<^bold>s\<^bold>e"
  proof -  
    have L7: "\<^bold>i\<^bold>n\<^bold>d \<^bold>p\<^bold>s\<^bold>e" using L1c L3 by blast
    have L8: "\<^bold>i\<^bold>n\<^bold>d \<^bold>p\<^bold>s\<^bold>s\<^bold>e" using L3 L7 by blast
