theory LiarsStreetDefinitions 
  (* imports BaseLogicClassical *)
  imports BaseLogicModalLogicK
begin
 (*Unimportant.*) 
...
consts Lives_in::"Kids\<Rightarrow>Streets\<Rightarrow>\<sigma>" ("_ lives-in _") 

consts SaysAccessibilityRel::"Kids\<Rightarrow>\<tau>"

definition  Says::"Kids\<Rightarrow>\<sigma>\<Rightarrow>\<sigma>"  ("_ says _") 
   where "X says \<phi> \<equiv> \<^bold>[SaysAccessibilityRel X\<^bold>] \<phi> "    

definition And ("_ and _") where "X and Y \<equiv> X \<^bold>\<and> Y"

definition Not ("not _") where "not X \<equiv> \<^bold>\<not>X"

definition If_then ("If _ then _") where "If X then Y \<equiv> X \<^bold>\<rightarrow> Y"
...
definition Lies ("lies _") 
  where "lies X \<equiv> \<^bold>\<forall>Y. If (X says Y) then not Y"
...
(*
definition forall_holds ("forall _ holds __") 
  where "forall X holds varphi \<equiv> \<^bold>\<forall>  If (X says Y) then Y" 
*) 
