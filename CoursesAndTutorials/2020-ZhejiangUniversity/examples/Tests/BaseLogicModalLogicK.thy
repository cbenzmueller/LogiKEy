...
abbreviation c8::"\<tau>\<Rightarrow>\<mu>" ("\<^bold>[_\<^bold>] _") where "\<^bold>[r\<^bold>] \<phi> \<equiv> \<lambda>w.\<forall>v.(r w v)\<longrightarrow>(\<phi> v)"
abbreviation c9::"\<tau>\<Rightarrow>\<mu>" ("\<^bold><_\<^bold>> _") where "\<^bold><r\<^bold>> \<phi> \<equiv> \<lambda>w.\<exists>v.(r w v)\<and>(\<phi> v)"
