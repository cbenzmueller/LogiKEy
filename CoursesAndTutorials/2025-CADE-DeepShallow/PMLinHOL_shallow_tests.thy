theory PMLinHOL_shallow_tests imports PMLinHOL_shallow                                          (* Christoph Benzmüller, 2025 *)
begin                 
...
\<comment>\<open>Reasoning with the Hilbert calculus: interactive and fully automated\<close>
  lemma HCderived1: "\<Turnstile>\<^sup>s (\<phi> \<supset>\<^sup>s \<phi>)" \<comment>\<open>sledgehammer(H1 H2 H3 MP) returns: by (metis H1 H2 MP)\<close>  
    proof -  have 1: "\<Turnstile>\<^sup>s \<phi> \<supset>\<^sup>s ((\<psi> \<supset>\<^sup>s \<phi>) \<supset>\<^sup>s \<phi>)" using H1 by auto 
...
\<comment>\<open>Correspondence theory: incorrect statements\<close> 
  lemma "reflexive R \<longrightarrow> (\<forall>\<phi> W V. \<forall>w:W. \<langle>W,R,V\<rangle>,w \<Turnstile>\<^sup>s \<box>\<^sup>s\<phi> \<supset>\<^sup>s \<box>\<^sup>s(\<box>\<^sup>s\<phi>))" nitpick[card \<w>=3] oops \<comment>\<open>nitpick: Ctex.\<close>
...
(**)  lemma "\<Turnstile>\<^sup>s \<box>\<^sup>s( \<box>\<^sup>s\<phi> \<supset>\<^sup>s  \<box>\<^sup>s\<psi>) \<or>\<^sup>s \<box>\<^sup>s( \<box>\<^sup>s\<psi> \<supset>\<^sup>s  \<box>\<^sup>s\<phi>)" nitpick[card \<w>=3] oops \<comment>\<open>nitpick: unknown\<close> 
...
\<comment>\<open>Implied axiom schemata in S5\<close>
  ...
  lemma K4B: "symmetric R \<and> transitive R 
                         \<longrightarrow> (\<forall>\<phi> \<psi> W V. \<forall>w:W. \<langle>W,R,V\<rangle>,w \<Turnstile>\<^sup>s \<box>\<^sup>s( \<box>\<^sup>s\<phi> \<supset>\<^sup>s  \<box>\<^sup>s\<psi>) \<or>\<^sup>s \<box>\<^sup>s( \<box>\<^sup>s\<psi> \<supset>\<^sup>s  \<box>\<^sup>s\<phi>))" by (smt DefS)
end
