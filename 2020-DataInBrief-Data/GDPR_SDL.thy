theory GDPR_SDL           (*GDPR CTD Example. C. Benzmüller & Xavier Parent, 2019*)
imports SDL
begin
...
(***Some Experiments***) 
 ...
 lemma False sledgehammer by (metis A0 F1 A1 A2 Implicit Situation D) (*Prove of Falsum.*)

(*Should the data be erased? — Yes, proof found by ATPs*)
 lemma "\<lfloor>\<^bold>\<circle><erase d1>\<rfloor>\<^sub>l"  sledgehammer nitpick by (metis A0 A2 F1 Situation) 
(*Should the data be kept? — Yes, proof found by ATPs*)
 lemma "\<lfloor>\<^bold>\<circle><\<^bold>\<not>erase d1>\<rfloor>\<^sub>l" sledgehammer nitpick by (meson A0 A1 F1 Implicit)
(*Should Mary be killed? — Yes, proof found by ATPs*)
 lemma "\<lfloor>\<^bold>\<circle><kill Mary>\<rfloor>\<^sub>l" sledgehammer nitpick by (meson A0 A1 A2 F1 Implicit Situation)      
end