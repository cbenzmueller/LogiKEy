...
axiomatization where 
CL1: "\<lfloor>appAnimal \<^bold>\<rightarrow> (STAB\<^sup>p \<^bold>\<prec> STAB\<^sup>d)\<rfloor>"
(* ... others as needed*)
...
lemma "\<lfloor>Pierson_facts \<^bold>\<and> For p\<rfloor>"
  nitpick[satisfy,show_all,card i=3] oops (*satisfiable? non-trivial model?*)
lemma "\<not>\<lfloor>Pierson_facts \<^bold>\<and> For p\<rfloor>"
  by (metis CW1 CW2 CW3 CW4 L2 R1 other.simps(2) rBR) (*only if using \<^bold>\<succ>\<^sub>E\<^sub>A variant*)
...
lemma assumes "\<lfloor>\<^bold>\<not>INCONS\<^sup>p\<rfloor>"
