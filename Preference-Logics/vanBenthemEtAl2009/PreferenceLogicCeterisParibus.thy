...
(*operators below are not defined in paper; existence is tacitly 
...
abbreviation c28 ("_\<^bold>\<preceq>\<^sub>A\<^sub>A\<^sup>__") where "\<phi> \<^bold>\<preceq>\<^sub>A\<^sub>A\<^sup>\<Gamma> \<psi> \<equiv> \<^bold>A(\<phi> \<^bold>\<and> [\<Gamma>]\<^sup>\<prec>\<^bold>\<not>\<psi>)"
...
(*Lines 80--92: Except for $<^\Gamma$, the operators defined here are not 
explicitly defined in the paper, but their existence is tacitly suggested. 
$<^\Gamma$ aims at modelling von Wright's notion of ceteris paribus (``all 
other things being equal'') preferences wrt. a set of formulas $\Gamma$ 
and it corresponds to $<_{AA}^\Sigma$ below, where $\Sigma=cp(\Gamma)$, 
i.e., $\Sigma$ are the propositional atoms independent from (not occurring 
in)  $\Gamma$, they are to be elicited by extra-logical means. Our variants 
below can thus be understood as "these (given) things being equal"-preferences.*)
