theory PALandWiseMenPuzzle2021 imports Main    (* Sebastian Reiche and Christoph Benzmüller, 2021 *)
...
begin
 ...
 (* alternatively, use set notation
 abbreviation  Agent::"\<alpha>\<Rightarrow>bool" ("\<A>") where "\<A> x \<equiv> x \<in> {a, b, c}" *)

 abbreviation EVR::"\<alpha>" ("EVR") where "EVR \<equiv> \<^bold>\<Union> \<A>"
 abbreviation DIS :: "\<alpha>" ("DIS") where "DIS \<equiv> \<^bold>\<Inter> \<A>"
 ...
 (* alldifferent doesn't seem to be neccessary
   and  alldifferent:  "\<not>(a = b) \<and> \<not>(a = c) \<and> \<not>(b = c)" *)
 ...
 -- "vampire": One-line proof reconstruction failed: by (metis bigintersection_rel_def) *)
 ...
 lemma mix_axiom1: "\<^bold>\<lfloor>\<^bold>C\<^bold>\<lparr>\<chi>\<^bold>|\<phi>\<^bold>\<rparr> \<^bold>\<rightarrow> (\<^bold>E\<^sub>\<A> (\<chi> \<^bold>\<rightarrow> (\<phi> \<^bold>\<and> \<^bold>C\<^bold>\<lparr>\<chi>\<^bold>|\<phi>\<^bold>\<rparr>)))\<^bold>\<rfloor>" unfolding Defs (* by metis *) oops
 ...
 lemma rkc_necessitation: assumes "\<^bold>\<lfloor>\<phi>\<^bold>\<rfloor>" shows "\<^bold>\<lfloor>\<^bold>C\<^bold>\<lparr>\<chi>\<^bold>|\<phi>\<^bold>\<rparr>\<^bold>\<rfloor>"
 ...
 lemma System_W: "\<A> i \<Longrightarrow> \<^bold>\<lfloor>(\<^bold>K\<^sub>i \<phi>) \<^bold>\<rightarrow> (\<^bold>D\<^sub>\<A> \<phi>)\<^bold>\<rfloor>" using bigintersection_rel_def by auto
 ...
 lemma System_T\<^sub>D: "\<^bold>\<lfloor>(\<^bold>D\<^sub>\<A> \<phi>) \<^bold>\<rightarrow> \<phi>\<^bold>\<rfloor>" using bigintersection_rel_def group_S5 reflexive_def by auto
 lemma System_4\<^sub>D: "\<^bold>\<lfloor>(\<^bold>D\<^sub>\<A> \<phi>) \<^bold>\<rightarrow> (\<^bold>D\<^sub>\<A> (\<^bold>D\<^sub>\<A> \<phi>))\<^bold>\<rfloor>" by (smt bigintersection_rel_def group_S5 transitive_def)
 lemma System_5\<^sub>D: "\<^bold>\<lfloor>(\<^bold>\<not>\<^bold>D\<^sub>\<A> \<phi>) \<^bold>\<rightarrow> (\<^bold>D\<^sub>\<A> (\<^bold>\<not>\<^bold>D\<^sub>\<A> \<phi>))\<^bold>\<rfloor>"
   by (smt bigintersection_rel_def euclidean_def group_S5)
 ...
 theorem "\<^bold>\<lfloor>\<^bold>[\<^bold>!\<^bold>\<not>\<^bold>K\<^sub>a(\<^sup>Aws a)\<^bold>](\<^bold>[\<^bold>!\<^bold>\<not>\<^bold>K\<^sub>b(\<^sup>Aws b)\<^bold>](\<^bold>K\<^sub>c (\<^sup>Aws c)))\<^bold>\<rfloor>" 
   using WM1 WM2ba WM2ca WM2cb group_S5 unfolding Defs by metis
