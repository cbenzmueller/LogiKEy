...
theory PAL_WiseMenPuzzle_test imports PAL_definitions
...
begin
 ...
 axiomatization where
   WM1: "\<^bold>\<lfloor>\<^bold>C\<^sub>\<A> (\<^sup>Aws a \<^bold>\<or> \<^sup>Aws b \<^bold>\<or> \<^sup>Aws c)\<^bold>\<rfloor>" 
 axiomatization where 
   (* Common knowledge: If x does not have a white spot then y know this *)
   WM2: "\<A> x \<and> \<A> y \<and> x \<noteq> y \<Longrightarrow> \<^bold>\<lfloor>\<^bold>C\<^sub>\<A> ((\<^bold>\<not>(\<^sup>Aws x)) \<^bold>\<rightarrow> (\<^bold>K\<^sub>y (\<^bold>\<not>(\<^sup>Aws x))))\<^bold>\<rfloor>"

 (* Positive introspection principles are implied *)
 lemma WM2': "\<A> x \<and> \<A> y \<and> x \<noteq> y \<Longrightarrow> \<^bold>\<lfloor>\<^bold>C\<^sub>\<A> ((\<^sup>Aws x) \<^bold>\<rightarrow> (\<^bold>K\<^sub>y (\<^sup>Aws x)))\<^bold>\<rfloor>"
   using WM2 unfolding Defs (* by (smt (z3) euclidean_def group_S5 reflexive_def) *) sorry
 
 lemma WM2ab': "\<^bold>\<lfloor>\<^bold>C\<^sub>\<A> ((\<^sup>Aws a) \<^bold>\<rightarrow> (\<^bold>K\<^sub>b (\<^sup>Aws a)))\<^bold>\<rfloor>"
   using WM1 WM2 WM2' group_S5 unfolding Defs oops
 (*
