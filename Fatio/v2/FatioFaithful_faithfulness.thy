...
primrec DpToShC :: "Formula\<Rightarrow>\<sigma>\<^sub>c" ("\<lfloor>_\<rfloor>\<^sup>c") where
    "\<lfloor>x\<^sup>c\<rfloor>\<^sup>c = x\<^sup>c\<^sup>s" | "\<lfloor>\<not>\<^sup>c\<phi>\<rfloor>\<^sup>c = \<not>\<^sup>c\<^sup>s\<lfloor>\<phi>\<rfloor>\<^sup>c" | "\<lfloor>\<phi> \<supset>\<^sup>c \<psi>\<rfloor>\<^sup>c = \<lfloor>\<phi>\<rfloor>\<^sup>c \<supset>\<^sup>c\<^sup>s \<lfloor>\<psi>\<rfloor>\<^sup>c"

theorem Faithful0a: "\<forall>V w. V,w \<Turnstile>\<^sup>c \<phi> \<longleftrightarrow> \<lfloor>\<phi>\<rfloor>\<^sup>c V w"
  apply (induct \<phi>) by auto

theorem Faithful0b: "\<Turnstile>\<^sup>c \<phi> \<longleftrightarrow> \<Turnstile>\<^sup>c\<^sup>s \<lfloor>\<phi>\<rfloor>\<^sup>c"
...
primrec DpToSh :: "BDF\<Rightarrow>\<sigma>" ("\<lparr>_\<rparr>") where
    "\<lparr>x\<^sup>d\<rparr> = x\<^sup>s" | "\<lparr>Done\<^sup>d l\<rparr> = Done\<^sup>s l" | "\<lparr>EntD \<Phi> sg \<phi>\<rparr> = EntS \<Phi> sg \<phi>"
...
proof (rule ConsD_I)
  fix W B D V U E w
  assume sat: "\<forall>\<gamma>. \<Gamma> \<gamma> \<longrightarrow> (\<forall>v:W. \<langle>W,B,D,V,U,E\<rangle>,v \<Turnstile>\<^sup>d \<gamma>)" and wW: "W w"
  from assms obtain \<Delta> where "\<Gamma> \<Turnstile> \<D>\<^sup>d j (\<B>\<^sup>d k (\<D>\<^sup>d j (Done\<^sup>d (Justify i \<Delta> sg \<phi>))))" ..
  from ConsD_E[OF this sat wW]
  show "\<langle>W,B,D,V,U,E\<rangle>,w \<Turnstile>\<^sup>d \<D>\<^sup>d j (\<B>\<^sup>d k (\<D>\<^sup>d j (ExJD i sg \<phi>)))" by simp blast
qed
...
primrec DpToShMin :: "BDF\<Rightarrow>\<sigma>\<^sub>m" ("\<lceil>_\<rceil>") where
    "\<lceil>x\<^sup>d\<rceil> = x\<^sup>m" | "\<lceil>Done\<^sup>d l\<rceil> = Done\<^sup>m l" | "\<lceil>EntD \<Phi> sg \<phi>\<rceil> = EntM \<Phi> sg \<phi>"
...
proof
  ...
  have "\<psi> = \<lceil>p\<^sup>d \<supset>\<^sup>d p\<^sup>d\<rceil>"
  proof (rule ext)
    fix w show "\<psi> w = \<lceil>p\<^sup>d \<supset>\<^sup>d p\<^sup>d\<rceil> w" using L unfolding ValM_def by simp
  qed
  moreover have "\<Turnstile>\<^sup>d (p\<^sup>d \<supset>\<^sup>d p\<^sup>d)" unfolding ValD_def by simp
...
lemma MinimalNotDeep_conditional:
  ...
  shows "\<Turnstile>\<^sup>m \<lceil>x\<^sup>d\<rceil>" and "\<not> \<Turnstile>\<^sup>d x\<^sup>d"
proof -
  show "\<Turnstile>\<^sup>m \<lceil>x\<^sup>d\<rceil>" using minimally_valid unfolding ValM_def by simp
next
  show "\<not> \<Turnstile>\<^sup>d x\<^sup>d"
    using elsewhere_false ValD_E[of "x\<^sup>d" "\<lambda>w. True" w\<^sub>1 B\<^sub>0 D\<^sub>0 V U\<^sub>0 E\<^sub>0] by auto
qed
