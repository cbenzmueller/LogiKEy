...
(* Audit convention: statements ending in oops are retained experiments or
   open questions, not exported theorems. Nitpick counterexample experiments
   deliberately test false converse implications. No statement is admitted. *)
...
lemma groundedExt_defEq1: "groundedExt\<^sup>\<A> att S \<longleftrightarrow> groundedExt2\<^sup>\<A> att S" 
  unfolding groundedExt_def groundedExt2_def
proof
  assume min: "minimal\<^sup>\<A> (completeExt\<^sup>\<A> att) S id"
  obtain L where least: "least\<^sup>\<A> (fixpoint\<^sup>\<A> (\<F>\<^sup>\<A> att)) L id"
    using \<F>_leastFP_ex by blast
  have cs: "completeExt\<^sup>\<A> att S"
    using min unfolding minimal_rel_def by blast
  have fps: "fixpoint\<^sup>\<A> (\<F>\<^sup>\<A> att) S"
    using cs completeExt_defEq unfolding completeExt2_def by blast
  have fpl: "fixpoint\<^sup>\<A> (\<F>\<^sup>\<A> att) L"
    using least unfolding least_rel_def by blast
  have ls: "L \<subseteq>\<^sup>\<A> S"
    using least fps unfolding least_rel_def id_def by blast
  have cfs: "conflictfreeExt\<^sup>\<A> att S"
    using cs unfolding completeExt_def admissibleExt_def by blast
  have cfl: "conflictfreeExt\<^sup>\<A> att L"
    using cfs ls unfolding conflictfreeExt_def by blast
  have cl: "completeExt\<^sup>\<A> att L"
    using fpl cfl completeExt_defEq unfolding completeExt2_def by blast
  have eq: "L \<approx>\<^sup>\<A> S"
    using min cl ls unfolding minimal_rel_def id_def by blast
  have lower: "\<forall>E. completeExt\<^sup>\<A> att E \<longrightarrow> S \<subseteq>\<^sup>\<A> E"
  proof (rule allI, rule impI)
    fix E assume ce: "completeExt\<^sup>\<A> att E"
    have fpe: "fixpoint\<^sup>\<A> (\<F>\<^sup>\<A> att) E"
      using ce completeExt_defEq unfolding completeExt2_def by blast
    have le: "L \<subseteq>\<^sup>\<A> E"
      using least fpe unfolding least_rel_def id_def by blast
    show "S \<subseteq>\<^sup>\<A> E" using le eq by blast
  qed
  show "least\<^sup>\<A> (completeExt\<^sup>\<A> att) S id"
    unfolding least_rel_def id_def by (rule conjI[OF cs lower])
next
  assume leastc: "least\<^sup>\<A> (completeExt\<^sup>\<A> att) S id"
  show "minimal\<^sup>\<A> (completeExt\<^sup>\<A> att) S id"
    by (rule leastMin_rel[THEN mp, OF leastc])
qed
...
(* Closing an ideal set under defended arguments remains below every preferred
   extension. Maximality therefore makes an ideal extension complete. *)
lemma idealExt_complete: "idealExt\<^sup>\<A> att S \<Longrightarrow> completeExt\<^sup>\<A> att S"
proof -
  assume ideal: "idealExt\<^sup>\<A> att S"
  have adm: "admissibleExt\<^sup>\<A> att S"
    using ideal unfolding idealExt_def greatest_rel_def idealSet_def by blast
  have closed: "idealSet\<^sup>\<A> att (S \<union> \<F>\<^sup>\<A> att S)"
    using ideal aux[OF adm]
    unfolding idealExt_def greatest_rel_def idealSet_def
    by (smt (verit) MONO_rel_def \<F>_mono completeExt_def maximal_rel_def preferredExt_def)
  have "\<F>\<^sup>\<A> att S \<subseteq>\<^sup>\<A> S"
    using ideal closed unfolding idealExt_def greatest_rel_def by auto
  with adm show ?thesis unfolding completeExt_def by blast
qed
...
lemma bcgProp11_3: \<open>completeExt\<^sup>\<A> att S \<and> (S \<union> [\<A>|att|S]\<^sup>+) \<approx>\<^sup>\<A> \<A> \<Longrightarrow> preferredExt\<^sup>\<A> att S \<and> (S \<union> [\<A>|att|S]\<^sup>+) \<approx>\<^sup>\<A> \<A>\<close>
  by (smt (verit, ccfv_threshold) admissibleExt_def completeExt_def conflictfreeExt_def preferredExt_def id_apply maximal_rel_def plusset_rel_def)
