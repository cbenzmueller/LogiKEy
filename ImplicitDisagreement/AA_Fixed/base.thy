...
(* Skeptical acceptance quantifies over every semantic labelling, including
   those with an empty In-set. For an empty collection of labellings this
   universal condition is vacuously true; see skeptical_credulous_nonempty. *)
...
lemma skeptical_credulous_nonempty:
  "(\<exists>Lab. S att Lab) \<Longrightarrow> sJ S att a \<Longrightarrow> cJ S att a"
  unfolding skepticallyJustified_def credulouslyJustified_def by blast
