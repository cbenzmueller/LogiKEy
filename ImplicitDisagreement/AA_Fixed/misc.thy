...
lemma minLeastCollapse:      \<open>\<exists>A. least Prop A E \<Longrightarrow> \<forall>B. minimal Prop B E \<longrightarrow>  least Prop B E\<close> 
  unfolding least_def minimal_def by blast
lemma minLeastCollapse_rel: \<open>\<exists>A. least\<^sup>U Prop A E \<Longrightarrow> \<forall>B. minimal\<^sup>U Prop B E \<longrightarrow>  least\<^sup>U Prop B E\<close> 
  unfolding least_rel_def minimal_rel_def by blast
lemma maxGreatestCollapse:      \<open>\<exists>A. greatest Prop A E \<Longrightarrow> \<forall>B. maximal Prop B E \<longrightarrow>  greatest Prop B E\<close> 
  unfolding greatest_def maximal_def by blast
lemma maxGreatestCollapse_rel: \<open>\<exists>A. greatest\<^sup>U Prop A E \<Longrightarrow> \<forall>B. maximal\<^sup>U Prop B E \<longrightarrow>  greatest\<^sup>U Prop B E\<close> 
  unfolding greatest_rel_def maximal_rel_def by blast
...
lemma sup_char_rel: \<open>least\<^sup>U (UB\<^sup>U S) (supremum S) id\<close>
  unfolding least_rel_def upper_bound_rel_def supremum_def by auto
lemma inf_char_rel: \<open>greatest\<^sup>U (LB\<^sup>U S) (infimum S) id\<close>
  unfolding greatest_rel_def lower_bound_rel_def infimum_def by auto
...
proof -
  assume mf: "MONO F"
  let ?S = "\<^bold>\<And>(\<lambda>X. F X \<subseteq> X)"
  have mono: "\<And>A B. A \<subseteq> B \<Longrightarrow> F A \<subseteq> F B"
    using mf unfolding MONO_def by blast
  have lower: "\<And>X. F X \<subseteq> X \<Longrightarrow> ?S \<subseteq> X"
    unfolding infimum_def by blast
  have pre: "F ?S \<subseteq> ?S"
  proof -
    have "\<And>X. F X \<subseteq> X \<Longrightarrow> F ?S \<subseteq> X"
      using mono lower by blast
    then show ?thesis unfolding infimum_def by blast
  qed
  have post: "?S \<subseteq> F ?S"
    by (rule lower[OF mono[OF pre]])
  have fp: "fixpoint F ?S" using pre post unfolding fixpoint_def by blast
  have least: "\<forall>X. fixpoint F X \<longrightarrow> ?S \<subseteq> X"
  proof (rule allI, rule impI)
    fix X assume "fixpoint F X"
    then have "F X \<subseteq> X" unfolding fixpoint_def by blast
    then show "?S \<subseteq> X" by (rule lower[of X])
  qed
  show ?thesis unfolding Let_def least_def id_def by (rule conjI[OF fp least])
qed
lemma wKTg': \<open>MONO F \<Longrightarrow> let S = \<^bold>\<Or>(\<lambda>X. X \<subseteq> F X) in (greatest (fixpoint F) S id)\<close>
proof -
  assume mf: "MONO F"
  let ?S = "\<^bold>\<Or>(\<lambda>X. X \<subseteq> F X)"
  have mono: "\<And>A B. A \<subseteq> B \<Longrightarrow> F A \<subseteq> F B"
    using mf unfolding MONO_def by blast
  have upper: "\<And>X. X \<subseteq> F X \<Longrightarrow> X \<subseteq> ?S"
    unfolding supremum_def by blast
  have post: "?S \<subseteq> F ?S"
  proof (intro allI impI)
    fix x assume sx: "?S x"
    obtain X where prefix: "X \<subseteq> F X" and xx: "X x"
      using sx unfolding supremum_def by blast
    have sub: "X \<subseteq> ?S" by (rule upper[OF prefix])
    have image: "F X \<subseteq> F ?S" by (rule mono[OF sub])
    show "F ?S x" using prefix xx image by blast
  qed
  have pre: "F ?S \<subseteq> ?S"
    by (rule upper[OF mono[OF post]])
  have fp: "fixpoint F ?S" using pre post unfolding fixpoint_def by blast
  have greatest: "\<forall>X. fixpoint F X \<longrightarrow> X \<subseteq> ?S"
  proof (rule allI, rule impI)
    fix X assume "fixpoint F X"
    then have "X \<subseteq> F X" unfolding fixpoint_def by blast
    then show "X \<subseteq> ?S" by (rule upper[of X])
  qed
  show ?thesis unfolding Let_def greatest_def id_def by (rule conjI[OF fp greatest])
qed
...
proof -
  assume mf: "MONO\<^sup>U F"
  let ?S = "\<^bold>\<And>(\<lambda>X. F X \<subseteq>\<^sup>U X)"
  have mono: "\<And>A B. A \<subseteq>\<^sup>U B \<Longrightarrow> F A \<subseteq>\<^sup>U F B"
    using mf unfolding MONO_rel_def by blast
  have lower: "\<And>X. F X \<subseteq>\<^sup>U X \<Longrightarrow> ?S \<subseteq> X"
    unfolding infimum_def by blast
  have pre: "F ?S \<subseteq>\<^sup>U ?S"
  proof -
    have "\<And>X. F X \<subseteq>\<^sup>U X \<Longrightarrow> F ?S \<subseteq>\<^sup>U X"
      using mono lower by blast
    then show ?thesis unfolding infimum_def by blast
  qed
  have post: "?S \<subseteq> F ?S"
    by (rule lower[OF mono[OF pre]])
  have fp: "fixpoint\<^sup>U F ?S" using pre post unfolding fixpoint_rel_def by blast
  have least: "\<forall>X. fixpoint\<^sup>U F X \<longrightarrow> ?S \<subseteq> X"
  proof (rule allI, rule impI)
    fix X assume "fixpoint\<^sup>U F X"
    then have "F X \<subseteq>\<^sup>U X" unfolding fixpoint_rel_def by blast
    then show "?S \<subseteq> X" by (rule lower[of X])
  qed
  show ?thesis unfolding Let_def least_def id_def by (rule conjI[OF fp least])
qed
...
proof -
  assume mf: "MONO\<^sup>U F"
  let ?S = "\<^bold>\<Or>(\<lambda>X. X \<subseteq>\<^sup>U F X)"
  have mono: "\<And>A B. A \<subseteq>\<^sup>U B \<Longrightarrow> F A \<subseteq>\<^sup>U F B"
    using mf unfolding MONO_rel_def by blast
  have upper: "\<And>X. X \<subseteq>\<^sup>U F X \<Longrightarrow> X \<subseteq> ?S"
    unfolding supremum_def by blast
  have post: "?S \<subseteq>\<^sup>U F ?S"
  proof (intro allI impI)
    fix x assume ux: "U x" and sx: "?S x"
    obtain X where prefix: "X \<subseteq>\<^sup>U F X" and xx: "X x"
      using sx unfolding supremum_def by blast
    have sub: "X \<subseteq> ?S" by (rule upper[OF prefix])
    have subrel: "X \<subseteq>\<^sup>U ?S" using sub by blast
    have image: "F X \<subseteq>\<^sup>U F ?S" by (rule mono[OF subrel])
    show "F ?S x" using prefix ux xx image by blast
  qed
  have pre: "F ?S \<subseteq> ?S"
    by (rule upper[OF mono[OF post]])
  have fp: "fixpoint\<^sup>U F ?S" using pre post unfolding fixpoint_rel_def by blast
  have greatest: "\<forall>X. fixpoint\<^sup>U F X \<longrightarrow> X \<subseteq> ?S"
  proof (rule allI, rule impI)
    fix X assume "fixpoint\<^sup>U F X"
    then have "X \<subseteq>\<^sup>U F X" unfolding fixpoint_rel_def by blast
    then show "X \<subseteq> ?S" by (rule upper[of X])
  qed
  show ?thesis unfolding Let_def greatest_def id_def by (rule conjI[OF fp greatest])
qed

lemma wKTl_rel: \<open>MONO\<^sup>U F \<Longrightarrow> \<exists>S. least (fixpoint\<^sup>U F) S id\<close>
proof -
  assume m: "MONO\<^sup>U F"
  have witness: "least (fixpoint\<^sup>U F) (infimum (\<lambda>X. F X \<subseteq>\<^sup>U X)) id"
    using wKTl_rel'[OF m] by (simp only: Let_def)
  show ?thesis by (rule exI[where x="infimum (\<lambda>X. F X \<subseteq>\<^sup>U X)"], rule witness)
qed
lemma wKTl_relw: \<open>MONO\<^sup>U F \<Longrightarrow> \<exists>S. least\<^sup>U (fixpoint\<^sup>U F) S id\<close>
proof -
  assume m: "MONO\<^sup>U F"
  obtain S where l: "least (fixpoint\<^sup>U F) S id" using wKTl_rel[OF m] by blast
  have witness: "least\<^sup>U (fixpoint\<^sup>U F) S id" by (rule least_rel_impl[THEN mp, OF l])
  show ?thesis by (rule exI[where x=S], rule witness)
qed
lemma wKTg_rel: \<open>MONO\<^sup>U F \<Longrightarrow> \<exists>S. greatest (fixpoint\<^sup>U F) S id\<close>
proof -
  assume m: "MONO\<^sup>U F"
  have witness: "greatest (fixpoint\<^sup>U F) (supremum (\<lambda>X. X \<subseteq>\<^sup>U F X)) id"
    using wKTg_rel'[OF m] by (simp only: Let_def)
  show ?thesis by (rule exI[where x="supremum (\<lambda>X. X \<subseteq>\<^sup>U F X)"], rule witness)
qed
lemma wKTg_relw: \<open>MONO\<^sup>U F \<Longrightarrow> \<exists>S. greatest\<^sup>U (fixpoint\<^sup>U F) S id\<close>
proof -
  assume m: "MONO\<^sup>U F"
  obtain S where g: "greatest (fixpoint\<^sup>U F) S id" using wKTg_rel[OF m] by blast
  have witness: "greatest\<^sup>U (fixpoint\<^sup>U F) S id" by (rule greatest_rel_impl[THEN mp, OF g])
  show ?thesis by (rule exI[where x=S], rule witness)
qed
...
lemma directedMaxGreatest: "directed A \<Longrightarrow> \<forall>B. maximal A B id \<longrightarrow>  greatest A B id"
  unfolding directed_def greatest_def id_apply maximal_def by blast
...
lemma chainDirected: "chain S \<longrightarrow> directed S"
  by (metis (full_types) chain_def directed_def)
lemma chainDirected_rel: "chain\<^sup>U S \<longrightarrow> directed\<^sup>U S"
  by (smt (verit) chain_rel_def directed_rel_def)
...
lemma omegaCompleteUB: "\<omega>-cpo A \<Longrightarrow> allChainsHaveUB A"
  by (metis allChainsHaveUB_def least_def omegacomplete_def sup_char)
lemma omegaCompleteUB_rel: "\<omega>-cpo\<^sup>U A \<Longrightarrow> allChainsHaveUB\<^sup>U A"
  by (smt (verit, ccfv_SIG) allChainsHaveUB_rel_def omegacomplete_rel_def supremum_def upper_bound_rel_def)
