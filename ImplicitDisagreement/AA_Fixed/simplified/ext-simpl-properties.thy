...
lemma groundedExt_defEq1: "groundedExt AF S \<longleftrightarrow> groundedExt2 AF S" 
  unfolding groundedExt_def groundedExt2_def
proof
  assume min: "minimal (completeExt AF) S id"
  obtain L where least: "least (fixpoint (\<F> AF)) L id"
    using \<F>_leastFP_ex by blast
  have cs: "completeExt AF S"
    using min unfolding minimal_def by blast
  have fps: "fixpoint (\<F> AF) S"
    using cs completeExt_defEq unfolding completeExt2_def by blast
  have fpl: "fixpoint (\<F> AF) L"
    using least unfolding least_def by blast
  have ls: "L \<subseteq> S"
    using least fps unfolding least_def id_def by blast
  have cfs: "conflictfreeExt AF S"
    using cs unfolding completeExt_def admissibleExt_def by blast
  have cfl: "conflictfreeExt AF L"
    using cfs ls unfolding conflictfreeExt_def by blast
  have cl: "completeExt AF L"
    using fpl cfl completeExt_defEq unfolding completeExt2_def by blast
  have eq: "L \<approx> S"
    using min cl ls unfolding minimal_def id_def by blast
  have lower: "\<forall>E. completeExt AF E \<longrightarrow> S \<subseteq> E"
  proof (rule allI, rule impI)
    fix E assume ce: "completeExt AF E"
    have fpe: "fixpoint (\<F> AF) E"
      using ce completeExt_defEq unfolding completeExt2_def by blast
    have le: "L \<subseteq> E"
      using least fpe unfolding least_def id_def by blast
    show "S \<subseteq> E" using le eq by blast
  qed
  show "least (completeExt AF) S id"
    unfolding least_def id_def by (rule conjI[OF cs lower])
next
  assume leastc: "least (completeExt AF) S id"
  show "minimal (completeExt AF) S id"
    by (rule leastMin[THEN mp, OF leastc])
qed
