...
axiomatization where (*axiomatization of the ethico-legal value ontology*)
 ...
 V3:"\<lfloor>((V x RELI) \<^bold>\<or> (V x STAB)) \<^bold>\<rightarrow> (UV x SECURITY)\<rfloor>" and
 V4:"\<lfloor>((V x EQUI) \<^bold>\<or> (V x FAIR)) \<^bold>\<rightarrow> (UV x EQUALITY)\<rfloor>" and
 V5:"\<lfloor>((V x RESP) \<^bold>\<or> (V x WILL)) \<^bold>\<rightarrow> (UV x LIBERTY)\<rfloor>" and
 V6:"\<lfloor>((V x GAIN) \<^bold>\<or> (V x EFFI)) \<^bold>\<rightarrow> (UV x UTILITY)\<rfloor>" 
...
lemma "\<not>(\<exists>w. ((FoxHunting d \<^bold>\<and> (Intent p \<^bold>\<and> Poss d)) \<^bold>\<and> For d) w)" 
  using R4 S2 V1 V3 V5 W2 reflBR by blast
...
lemma "\<lfloor>(WildAnimals p \<^bold>\<and> (WildAnimals d \<^bold>\<and> (Liv p \<^bold>\<and> Land p))) \<^bold>\<rightarrow> For p\<rfloor>" 
