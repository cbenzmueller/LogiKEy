...
abbreviation leAE_cp::\<epsilon>   ("_\<prec>\<^sub>A\<^sub>E\<^sup>__") (*AE-variant *) 
  where "(\<phi> \<prec>\<^sub>A\<^sub>E\<^sup>\<Gamma> \<psi>) u \<equiv> \<forall>s.\<exists>t. (\<phi> s) \<longrightarrow> (\<psi> t)\<and>(s \<^bold>\<lhd>\<^sub>\<Gamma> t)" 
abbreviation leqAE_cp::\<epsilon>  ("_\<preceq>\<^sub>A\<^sub>E\<^sup>__") 
