...
(*...*)
lemma True nitpick[satisfy] oops (*axioms consistent - what is "Empty assignment"? *)
...
lemma "let \<Gamma> = VAL \<^bold>\<midarrow> \<^bold>{WILL \<alpha>,STAB \<beta>\<^bold>}
                   in \<lfloor>FoxHunting \<^bold>\<rightarrow> (WILL \<alpha> \<^bold>\<preceq>\<^sub>A\<^sub>E\<^sup>\<Gamma> STAB \<beta>)\<rfloor>" using S2 W3 by fastforce 
