theory DDLlewis imports Main

begin

(*** We introduce lewis's system ***)
...
theorem T3: 
  ...
  shows "Suzumura"
<<<<<<< HEAD:Deontic-Logics/cube-ddl/DDLcube.thy~
  sledgehammer
  by (metis assms sub_rel_def tcr_def transitive_def)
 
=======
  by (metis assms sub_rel_def tcr_def transitive_def) 
>>>>>>> deb0d62ba60065659b5133ee33ad576686e8cee8:Deontic-Logics/cube-ddl/old/DDLlewis.thy
...
(* axioms of E holding if \<^bold>r transitive and/or total *)
...
theorem T30:
  assumes transitivity
  shows tra'': "\<lfloor>( \<integral><\<phi>|(\<phi>\<^bold>\<or>\<psi>)> \<^bold>\<and> \<integral><\<psi>|(\<psi>\<^bold>\<or>\<xi>)>) \<^bold>\<rightarrow> \<integral><\<phi>|(\<phi>\<^bold>\<or>\<xi>)> \<rfloor>"    
  sledgehammer oops
