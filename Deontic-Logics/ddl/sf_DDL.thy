theory sf_DDL imports Main            (*Xavier Parent*)

begin (* DDL: Dyadic Deontic Logic with selection functions *)
...
(*  ax_5a: "\<forall>X. ob(X)(\<lambda>x. True)" and *)
  ax_1: "\<forall>X. ob(X)(X)" and (*identity*)
  ax_2: "\<forall>X Y Z. (\<forall>w. ((Y(w)) \<longleftrightarrow> (Z(w)))) \<longrightarrow> (ob(X)(Y) \<longleftrightarrow> ob(X)(Z))" and (*extension*)
  ax_3: "\<forall>X \<beta>. ((\<forall>Z. \<beta>(Z) \<longrightarrow> ob(X)(Z)) \<and> (\<exists>Z. \<beta>(Z))) \<longrightarrow> 
